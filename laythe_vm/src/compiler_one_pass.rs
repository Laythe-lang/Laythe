...
pub struct CompilerOnePass<'a, 's> {
...
impl<'a, 's> CompilerOnePass<'a, 's> {
  ...
  /// # Examples
  /// ```
  /// use laythe_vm::compiler_one_pass::{CompilerOnePass, ParserOnePass};
  ...
  /// let mut parser = ParserOnePass::new(Stdio::default(), &source);
  /// let module = hooks.manage(Module::new(
  ///  hooks.manage(Class::bare(hooks.manage_str("module"))),
  ///  hooks.manage(PathBuf::from("./module.ly"))
  /// ));
  ///
  /// let compiler = CompilerOnePass::new(module, &mut parser, &mut hooks);
  /// ```
  pub fn new(
    module: Managed<Module>,
    parser: &'a mut ParserOnePass<'s>,
  ...
  fn child(
    ...
    fun_kind: FunKind,
    enclosing: *mut CompilerOnePass<'a, 's>,
  ) -> Self {
    let mut child = Self {
  ...
  fn function(&mut self, fun_kind: FunKind) {
    ...
    let mut fun_compiler = CompilerOnePass::child(name, fun_kind, &mut *self);
    ...
    while !self.parser.check(TokenKind::RightBrace) {
      ...
      if entries == std::u16::MAX as usize {
        self.parser.error(&format!(
          "Cannot have more than {} key value paOirs in map literal",
          entries
        ));
      }
      entries += 1;
  ...
  fn lambda(&mut self) {
    ...
    let mut fun_compiler = CompilerOnePass::child(name, FunKind::Fun, &mut *self);
    fun_compiler.begin_scope();
...
const RULES_TABLE: [ParseRule; 54] = [
...
mod test {
  ...
  fn test_compile<'a>(src: String, context: &dyn GcContext) -> Managed<Fun> {
    ...
    let mut parser = ParserOnePass::new(stdio, &src);

    let hooks = &GcHooks::new(context);

    let module = hooks
      .manage(Module::from_path(&hooks, hooks.manage(PathBuf::from("path/module.ly"))).unwrap());
    let compiler = CompilerOnePass::new(module, &mut parser, &hooks);
    let result = compiler.compile();
