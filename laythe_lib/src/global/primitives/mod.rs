...
use laythe_core::{hooks::GcHooks, module::Module, package::Package, ModuleResult, LyError};
...
fn bootstrap_classes(
  ...
  let mut object_class = match module.get_symbol(hooks.manage_str(OBJECT_CLASS_NAME.to_string())) {
    ...
    None => return Err(LyError::new(hooks.manage_str("Could not find Laythe class object in std_lib construction.".to_string()))),
  };
  ...
  let mut class_class = match module.get_symbol(hooks.manage_str(CLASS_CLASS_NAME.to_string())) {
    Some(class) => class.to_class(),
    None => return Err(LyError::new(hooks.manage_str("Could not find Laythe class class in std_lib construction.".to_string()))),
  };
  ...
  let mut object_meta_class = create_meta_class(hooks, object_class.name, class_class);
